  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      if (response.data?.length === 0) {
        // No orders found, but not an error
        setError(null);
      }
    } catch (err) {
...
function OrderCard({ order, formatDate, getStatusConfig }) {
  ...
  return (
    <div className="border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow bg-white">
      {/* Order Header */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4 pb-4 border-b">
        <div>
      ...
      {/* Order Footer */}
      <div className="flex flex-col sm:flex-row justify-between items-end sm:items-center gap-4 mt-4 pt-4 border-t">
  ...
  if (!product) {
    return (
      <div className="flex justify-between border-b pb-3">
        <div className="text-gray-500">Product information unavailable</div>
        <div className="font-semibold">R{itemTotal.toFixed(2)}</div>
      </div>
    );
  }
